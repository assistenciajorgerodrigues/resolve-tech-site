...
const PASSWORD_HASH_B64 = "vtyEJM4Z5nICwC5v2suebRao9WrVmfzv+kq3quTd3sY=";
...
export async function ensureAuthSchema() {
  await db().batch([
    db().prepare(
      "CREATE TABLE IF NOT EXISTS admin_sessions (token TEXT PRIMARY KEY, created_at TEXT NOT NULL, expires_at TEXT NOT NULL)"
    ),
  ]);
}
...
export async function createAdminSession() {
  ...
  const expires = new Date(
    created.getTime() + SESSION_TTL_SECONDS * 1000
  );

  await db()
    .prepare("DELETE FROM admin_sessions WHERE expires_at <= ?")
    .bind(created.toISOString())
    .run();
