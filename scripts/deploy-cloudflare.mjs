...
function runJson(args) {
  try {
    const output = execFileSync(process.execPath, [wrangler, ...args], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }).trim();
    const start = Math.min(...[output.indexOf("{"), output.indexOf("[")].filter((v) => v >= 0));
...
function findD1Id(value) {
  ...
  if (Array.isArray(value)) {
    for (const item of value) { const found = findD1Id(item); if (found) return found; }
    return "";
  }
  ...
  if (binding === "DB" || type.includes("d1")) {
    ...
    for (const child of Object.values(value)) {
      ...
      const found = findD1Id(child); if (found) return found;
    }
  }
  for (const child of Object.values(value)) { const found = findD1Id(child); if (found) return found; }
  return "";
}

function versionIds(value, out = []) {
  if (!value || typeof value !== "object") return out;
  if (Array.isArray(value)) { for (const item of value) versionIds(item, out); return out; }
  for (const [key, child] of Object.entries(value)) {
    if (typeof child === "string" && key.toLowerCase().includes("version") && uuidPattern.test(child)) out.push(child);
    else versionIds(child, out);
  }
  return [...new Set(out)];
}
...
// Prefer the binding already attached to the currently deployed Worker. This only
// needs Workers access, so it works with the normal Workers Builds token.
if (!databaseId) {
...
// Fallback when the build token has D1 metadata permission.
if (!databaseId) {
...
const deploy = spawnSync(process.execPath, [wrangler, "deploy", "--config", configPath], { stdio: "inherit" });
